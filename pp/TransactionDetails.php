<?php  
// Only admins can execute this script. Header.php enforces this.
$isadminrequired = true;
$title = "PayPal ePayment Transaction Details"; 
$bError = false;
include_once('../header-minimal.php');

/******************************************************
TransactionDetails.php

Sends a GetTransactionDetails NVP API request to PayPal.

The code retrieves the transaction ID and constructs the
NVP API request string to send to the PayPal server. The
request to PayPal uses an API Signature.

After receiving the response from the PayPal server, the
code displays the request and response in the browser. If
the response was a success, it displays the response
parameters. If the response was an error, it displays the
errors received.

Called by GetTransactionDetails.html.

Calls CallerService.php and APIError.php.

******************************************************/

require_once 'CallerService.php';
$transactionID=urlencode($_REQUEST['transactionID']);

/* Construct the request string that will be sent to PayPal.
   The variable $nvpstr contains all the variables and is a
   name value pair string with & as a delimiter */
$nvpStr="&TRANSACTIONID=$transactionID";

/* Make the API call to PayPal, using API signature.
   The API response is stored in an associative array called $resArray */
$resArray=hash_call("gettransactionDetails",$nvpStr);

/* Next, collect the API request in the associative array $reqArray
   as well to display back to the browser.
   Normally you wouldnt not need to do this, but its shown for testing */

$reqArray=$_SESSION['nvpReqArray'];

/* Display the API response back to the browser.
   If the response from PayPal was a success, display the response parameters'
   If the response was an error, display the errors received using APIError.php.
   */
$ack = strtoupper($resArray["ACK"]);

if($ack!="SUCCESS"){
		$_SESSION['reshash']=$resArray;
		$location = "APIError.php";
		header("Location: $location");
	}

?>

<html>
<head>
    <title>Transaction details</title>
    <link href="sdk.css" rel="stylesheet" type="text/css" />
</head>
<body>
	<br>
		<center>
		<font size=3 color=black face=Verdana><b>Transaction Details</b></font>
		<br><br>

	<table width=400>
		<tr>
			<td>Date:</td>
 			<td><?php echo substr($resArray['ORDERTIME'], 0, 10) ?></td>
 		</tr>
			<td>Payer Email:</td>
 			<td><?php echo $resArray['EMAIL'] ?></td>
 		</tr>
		<tr>
			<td>Payer ID:</td>
			<td><?php echo $resArray['PAYERID'] ?></td>
		</tr>
		<tr>
			<td>First Name:</td>
			<td><?php echo $resArray['FIRSTNAME'] ?></td>
		</tr>
		<tr>
			<td>Last Name:</td>
			<td><?php echo $resArray['LASTNAME'] ?></td>
		</tr>
		<tr>
			<td>Transaction ID:</td>
			<td><?php echo $resArray['TRANSACTIONID']?></td>
		</tr>
<?php
	if (isset($resArray['PARENTTRANSACTIONID'])){ ?>		
		<tr>
			<td>Parent Transaction ID (if any):</td>
			<td><?php echo $resArray['PARENTTRANSACTIONID'] ?></td>
		</tr>
<?php } ?>
		<tr>
			<td>Gross Amount:</td>
			<td><?php echo $resArray['AMT']?></td>
		</tr>
		<tr>
			<td>Fee Amount:</td>
 			<td><?php echo $resArray['FEEAMT'] ?></td>
 		</tr>
		<tr>
			<td>Net Amount:</td>
			<td><?php echo $resArray['AMT']-$resArray['FEEAMT']?></td>
		</tr>
<?php 
	if ((isset($resArray['RECURRING'])) && ($resArray['RECURRING'] == 1 )){ ?>
		<tr>
			<td>Subscription Length</td>
 			<td><?php echo substr($resArray['L_PERIOD2'], 0, strpos($resArray['L_PERIOD2'], ' '));?></td>
 		</tr><?php
	}

	if (isset($resArray['L_NUMBER0'])){ ?>
<tr>
<td>Number of classes</td>
<td><?php	switch ($resArray['L_NUMBER0']) {
			case 'kids12-3m':
				$numclasses = 12;
				break;
			case 'kids24-6m':
				$numclasses = 24;
				break;
			case 'kids48-12m':
				$numclasses = 48;
				break;
			case 'adult18-3m':
				$numclasses = 18;
				break;
			case 'adult24-3m':
				$numclasses = 24;
				break;
			case 'adult36-3m':
				$numclasses = 36;
				break;
			default:
				$numclasses = 1;
				break;
		}
		echo $numclasses;?></td>
</tr>
<tr>
<?php } ?>
<td>Payment Status:</td>
<td><?php echo $resArray['PAYERSTATUS']?></td>
</tr>
<tr>
<td>Pending Reason:</td>
<td><?php echo $resArray['PENDINGREASON'] ?></td>
</tr>
<tr>
<td>ProtectionEligibility:</td>
<td><?php echo $resArray['PROTECTIONELIGIBILITY'] ?></td>
</tr>
</table>
<h4>Raw transaction detail</h4>
<p>
<?php
/*		$tran_ID=$resArray['TRANSACTIONID'];
		$currency_cd=$resArray['CURRENCYCODE'];
		$gross_amt=$resArray['AMT'];

		// Build links
		$do_void_link = 'DoVoid.php?authorization_id='.$tran_ID;
		$do_authorization_link = 'DoAuthorization.php?order_id='.$tran_ID.'&currency='.$currency_cd.'&amount='.$gross_amt;
		$do_capture_link = 'DoCapture.php?authorization_id='.$tran_ID.'&currency='.$currency_cd.'&amount='.$gross_amt;
		$do_refund_link = 'RefundTransaction.php?transaction_id='.$tran_ID.'&currency='.$currency_cd.'&amount='.$gross_amt;
*/		
		print_r($resArray);?>
<!--    <br> <font size=2 color=black face=Verdana>
    <a id="DoVoidLink" href="<?php echo $do_void_link?>">Void</a>
    <a id="DoAuthorization" href="<?php echo $do_authorization_link?>">Authorization</a>
    <a id="DoCaptureLink" href="<?php echo $do_capture_link?>">Capture</a>
    <a id="RefundTransactionLink" href="<?php echo $do_refund_link?>">Refund</a>
    <a id="BackLink" href="javascript:history.back()">Back</a>
    <br /> </font></center>
    <a class="home" id="CallsLink" href="index.html">Home</a>
-->
</p>
<input type="button" value="Return to previous page" class="btn" onmouseover="this.className='btn btnhover'" onmouseout="this.className='btn'" onclick="document.location.href = '<?php echo $_SERVER["HTTP_REFERER"];?>'"/>
<?php 
// Start footer section
include('../footer.php'); ?>
</body>
</html>