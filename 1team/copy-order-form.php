...
$bError = false;
// This is the order id of the order we are copying
if (isset($_GET["id"])) {
	$orderid = $_GET["id"];
} else {
	$bError = true;
...
<td width="20%"><input type="text" id="amount"cal name="amount" value="0.00" style="width: 8em" dojoType="dijit.form.CurrencyTextBox" required="true" constraints="{fractional:true}" currency="USD" invalidMessage="Invalid amount.  Include dollars and cents." /></td>
...
order[<?php echo $countOrders?>][<?php echo OrderItemArrayIndex_SKU?>] = <?php echo $orderResults[$countOrders]["skuid"]?>;
order[<?php echo $countOrders?>][<?php echo OrderItemArrayIndex_Amount?>] = <?php echo $orderResults[$countOrders]["amount"]?>;
...
function addrow(tableid, tablerow, skuformid, amtformid, feeformid){
...
debugOrder('before add');
	// New order array element
	order[++orderitemidx] = new Array(3);
	...
	order[orderitemidx][<?php echo OrderItemArrayIndex_SKU?>] = elmsku.options[elmsku.selectedIndex].value;
	...
	order[orderitemidx][<?php echo OrderItemArrayIndex_Amount?>] = floatamt;
	...
	order[orderitemidx][<?php echo OrderItemArrayIndex_Fee?>] = floatfee;
	...
	document.copyorderform.order.value = order.toString();
	document.copyorderform.numorderitems.value = ++numorderitems;
...
function isPaidChanged(){
	if (document.copyorderform.ispaid.value == "on"){
		hideit('duedatediv');
...
<td width="20%">
<div id="printinvoicediv" <?php if ($orderResults[0]["ispaid"]) echo 'class="hideit"';?>>
...
<div id="printreceiptdiv" <?php if (!$orderResults[0]["ispaid"]) echo 'class="hideit"';?>>
