...
try{
	$dbh = getDBH();	// This is the one time we call getDBH with no session, since it doesn't exist yet. 
...
$pdostatement->execute(array($formLogin));
...
if ($rowCount != 1) {
	...
	// Good password		
		...
		if ((doesRoleContain($roleid, Role_ApplicationAdmin)) && (!isAdminSystem()) ){ 
			...
			$sessionkey = createSession( $dbh, $userid, $roleid);
...
function createSession( $dbh, $userid, $roleid){
	...
	$rc = saveSession( $dbh, $sessionkey, $userid, $ipaddr);
	if (!isSuccessful($rc)) {
		return $rc;
	}
...
function saveSession( $dbh, $sessionkey, $userid, $ipaddr) {
	...
	$userloginResults = $pdostatement->fetchAll();
	...
	$pdostatement = $dbh->prepare($strSQL);
	$pdostatement->execute(array($sessionkey, $userid));
	
	$sessionResults = $pdostatement->fetchAll();
	...
	if ($rowCountSessions == 0) {
		...
		$pdostatement = $dbh->prepare($strSQL);
		$pdostatement->execute(array($ipaddr, $userid, $sessionkey, $login, $roleid, $fullname, $teamid, $isbillable, $status));
	...
	// Else get the session results
		...
		$pdostatement = $dbh->prepare($strSQL);
		$pdostatement->execute(array($login, $roleid, $fullname, $teamid, $isbillable, $status, $ipaddr, $userid, $teamid));
	}
	
	return(RC_Success);
}

function isUserLocationRecognized( $dbh,  $userid, $teamid, $ipaddr){
	$strSQL = "SELECT count(*) FROM recognizeduserlocations WHERE userid = ? AND teamid = ? AND ipaddr = ?;";
	$pdostatement = $dbh->prepare($strSQL);
	$pdostatement->execute(array($userid, $teamid, $ipaddr));
