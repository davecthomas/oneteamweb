...
if (!$bError) {
	...
	if ($bhasdisplaycondition) { 
		...
		$bError = !$pdostatement->execute(array($name, $datatype, $displayconditionobject, $displayconditionfield, $displayconditionoperator, $displayconditionvalue, $customlistid, $fieldid, $teamid));
	} else { 
		...
		$pdostatement = $dbh->prepare($strSQL);
		$bError = !$pdostatement->execute(array($name, $datatype, $customlistid, $fieldid, $teamid));
	}
